...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.is_pressed():
		...
		if mouse_event.button_index == MouseButton.MOUSE_BUTTON_LEFT: # open tile cell up / detect nearby cells with array
			left_click.emit(tile_array_pos, tile_pos)
			
		if mouse_event.button_index == MouseButton.MOUSE_BUTTON_RIGHT: # place down flag / remove flag
